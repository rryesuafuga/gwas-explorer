...
class GWASExplorerApp {
    ...
    setupEventListeners() {
        ...
        this.elements.btnDownloadManhattan?.addEventListener('click', () => {
            this.manhattanPlot?.downloadSVG();
        });
        ...
        this.elements.thresholdBtns.forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                this.manhattanPlot?.updateThreshold(this.significanceThreshold);
            });
        });
        ...
        this.elements.colorSchemeSelect?.addEventListener('change', (e) => {
            ...
            this.manhattanPlot?.updateColorScheme(this.colorScheme);
        });
    ...
    initializeVisualizations() {
        ...
        if (this.elements.manhattanPlot) {
            this.manhattanPlot = new ManhattanPlot('manhattan-plot', {
                ...
                onSNPClick: (snp) => this.handleSNPClick(snp),
                onSNPHover: (snp, event) => this.handleSNPHover(snp, event),
                onSNPLeave: () => this.hideTooltip()
            });
        }
        ...
        if (this.elements.qqPlot) {
            this.qqPlot = new QQPlot('qq-plot', {
                onPointHover: (data, event) => this.showTooltip(
                    `Expected: ${data.expected.toFixed(2)}<br>Observed: ${data.observed.toFixed(2)}`,
                    event
                ),
                onPointLeave: () => this.hideTooltip()
            });
        }
    ...
    async loadDemoData() {
        ...
        try {
            // Simulate GWAS data
            this.gwasData = this.simulator.generateGWAS({
                numSnps: 50000,
                numSignificant: 12,
                genomicInflation: 1.02
            });
            
            // Generate PRS weights
            this.prsWeights = this.simulator.generatePRSWeights(20);
    ...
    async simulateNewGWAS() {
        ...
        btn.disabled = true;
        btn.querySelector('.btn-icon').style.animation = 'spin 1s linear infinite';
        
        try {
            // Generate new random parameters
            const numSnps = 30000 + Math.floor(Math.random() * 40000);
            const numSignificant = 5 + Math.floor(Math.random() * 15);
    ...
    async updateVisualizations() {
        ...
        if (this.manhattanPlot) {
            await this.manhattanPlot.render(this.gwasData);
        }
        
        // Update QQ plot
        if (this.qqPlot) {
            const pvalues = this.gwasData.map(d => d.pval);
            await this.qqPlot.render(pvalues);
            this.updateQQStats(pvalues);
        }
    ...
    updateStats() {
        ...
        const significant = this.gwasData.filter(d => d.pval < this.significanceThreshold);
        const pvalues = this.gwasData.map(d => d.pval);
        const lambda = this.simulator.calculateLambda(pvalues);
        ...
        this.elements.statLambda.textContent = lambda.toFixed(3);
    }
    
    /**
     * Update QQ plot statistics panel
     */
    updateQQStats(pvalues) {
        const lambda = this.simulator.calculateLambda(pvalues);
        const lambda1000 = 1 + (lambda - 1) * (1 / pvalues.length) * 1000;
        
        // Calculate mean chi-squared
        const chiSquares = pvalues.map(p => {
            const z = -this.simulator.normalQuantile(p / 2);
            return z * z;
        });
        const meanChi2 = chiSquares.reduce((a, b) => a + b, 0) / chiSquares.length;
        
        // Calculate tail deviation
        const sortedP = [...pvalues].sort((a, b) => a - b);
        const topP = sortedP.slice(0, Math.floor(pvalues.length * 0.01));
        const expectedTop = topP.map((_, i) => (i + 1) / pvalues.length);
        const deviation = topP.reduce((sum, p, i) => {
            return sum + Math.abs(-Math.log10(p) - (-Math.log10(expectedTop[i])));
        }, 0) / topP.length;
        ...
        this.elements.qqDeviation.textContent = deviation.toFixed(2);
    ...
    populateGenotypeGrid() {
        if (!this.prsWeights || !this.elements.genotypeGrid) return;
        
        this.elements.genotypeGrid.innerHTML = this.prsWeights.map(variant => `
            <div class="genotype-item">
                <span class="genotype-snp">${variant.snp}</span>
                <input type="number" 
                       class="genotype-input" 
                       data-snp="${variant.snp}"
                       min="0" max="2" step="1" 
                       value="0"
                       placeholder="0-2">
            </div>
     ...
     */
    simulateGenotypes() {
        if (!this.prsWeights) return;
        
        const population = this.elements.simPopulation.value;
        const genotypes = this.simulator.simulateGenotypes(this.prsWeights, population);
        
        // Update input fields
        Object.entries(genotypes).forEach(([snp, dosage]) => {
            const input = this.elements.genotypeGrid.querySelector(`input[data-snp="${snp}"]`);
            if (input) {
                input.value = dosage;
                // Animate the change
     ...
     */
    async calculatePRS() {
        if (!this.prsCalculator || !this.prsWeights) return;
        
        // Collect genotypes from inputs
        const genotypes = {};
        this.elements.genotypeGrid.querySelectorAll('.genotype-input').forEach(input => {
            genotypes[input.dataset.snp] = parseInt(input.value) || 0;
        });
        
        try {
            const result = await this.prsCalculator.calculate(genotypes, this.prsWeights);
    ...
    populateQuickLoci() {
        ...
        const significant = this.gwasData
            .filter(d => d.isLead && d.pval < this.significanceThreshold)
        ...
        this.elements.quickLoci.innerHTML = significant.map(snp => `
            <button class="locus-chip" 
                    data-chr="${snp.chr}" 
                    data-pos="${snp.pos}"
                    title="${snp.snp} (p=${snp.pval.toExponential(2)})">
        ...
        this.elements.quickLoci.querySelectorAll('.locus-chip').forEach(chip => {
            chip.addEventListener('click', () => {
                ...
                const window = 500000; // 500kb window
                this.viewLocusRegion(chr, pos - window, pos + window);
            });
        });
    }
    ...
    handleSNPClick(snp) {
        ...
        // Update info panel
        document.getElementById('info-snp-id').textContent = snp.snp;
        document.getElementById('info-chr').textContent = snp.chr;
        document.getElementById('info-pos').textContent = snp.pos.toLocaleString();
        document.getElementById('info-pval').textContent = snp.pval.toExponential(2);
        document.getElementById('info-logp').textContent = snp.logP.toFixed(2);
        document.getElementById('info-maf').textContent = snp.maf?.toFixed(3) || '-';
        document.getElementById('info-beta').textContent = snp.beta?.toFixed(4) || '-';
        document.getElementById('info-gene').textContent = snp.gene || '-';
        
        // Update external link
        const ensemblLink = `https://www.ensembl.org/Homo_sapiens/Location/View?r=${snp.chr}:${snp.pos - 50000}-${snp.pos + 50000}`;
        document.getElementById('btn-external-link').href = ensemblLink;
        
        // Show panel
        this.elements.snpInfoPanel.classList.add('visible');
        
        // Set up LocusZoom button
        document.getElementById('btn-zoom-locus').onclick = () => {
    ...
    handleSNPHover(snp, event) {
        const content = `
            <div class="tooltip-title">${snp.snp}</div>
            <div class="tooltip-content">
                Chr${snp.chr}:${snp.pos.toLocaleString()}<br>
                P = ${snp.pval.toExponential(2)}<br>
                -log₁₀(P) = ${snp.logP.toFixed(2)}
